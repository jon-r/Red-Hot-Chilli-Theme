...
function questionToText(a){a=$.parseJSON(a);$queryModaloutput.removeClass("loading").html(a.answer).next("button").html(a.next)}var $form=$(".js_contact_form"),$formInputsReq=$form.find("input.req"),$formInputsOptional=$form.find("input:not(.req)"),$formNextBtn=$form.find(".js_nextBtn"),$formBackBtn=$form.find(".js_backBtn"),formErrorList=[];$form.attr("novalidate","");
...
$formNextBtn.click(function(a){$thisSubForm=$(this).parent("p.subform-active");formErrorList=[];$response=$thisSubForm.find(".response");$thisSubForm.find($formInputsReq).each(function(){$el=$(this);formValidate($el)});$response.removeClass("success").removeClass("error").empty();console.log("click");0==formErrorList.length?$thisSubForm.removeClass("subform-active").next("p").addClass("subform-active"):$response.addClass("error").html("Please fill in required items")});$formBackBtn.click(function(a){$(this).parent("p.subform-active").removeClass("subform-active").prev("p").addClass("subform-active")});
$formInputsReq.change(function(a){$el=$(this);formValidate($el)});$formInputsOptional.change(function(a){$el=$(this);0<$el.val().length?$el.addClass("success"):$el.removeClass("success")});
...
a.addClass("success")}function formAjaxReply(a){a=$.parseJSON(a);$resultOutcome="Form mailed successfully"==a?"success":"error";$form.find(".response").removeClass("loading").addClass($resultOutcome).html(a)};
